...
- [too bothersome] improve fix for matrix chart: when fixing, I assumed I need to apply the callbacks dynamically in renderer.js instead of doing it generically through hotsdata.js like with the other charts.
...
- [DONE?]better user guidance / step by step explanations and buttons
- [DONE] implement frontend feedback during I/O intense operations about their progress (processing x out of y replays to JSON; DB input operations)
- implement selectors (dropdown?) to limit chart display to
    - x number of last games
    - any games since date/time y
    - games on map z
- display top 3 heroes for a given map (for all maps?)
- implement toggle to consider only replay owner performance / all recorded performance
