...
**Download the latest windows release** for free from [GitHub Releases](https://github.com/pi11e/HOTSVIZ_Desktop/releases)
...
**Coaching Corner.** A short summary of what's worth your attention right now, from recent vs overall form, and targeted scope insights when you drill into a single hero or map.
...
| *Draft-time recommendations based on your win rate* | *Full breakdown of your matches, with post-game stats and details for both teams* |
...
| *From replays folder to full dashboard in seconds* | *Filter controls, summary and coaching corner* |
...
Click **More info → Run anyway** to proceed. The application is safe to use: 
- It only reads the files you tell it to read. 
- It cannot read anything other than .stormreplay files. 
- It makes no network calls outside of the auto-updater, which you can opt-out of if you prefer.
- It collects no usage data.
- It comes with proper (un)installer so you can easily remove it and all related data when needed.
...
- Data stored locally in SQLite at %APPDATA%\HOTSVIZ\data\hotsviz.db
